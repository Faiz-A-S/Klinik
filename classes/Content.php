...
Class Content extends DBConnection {
	...
	public function client(){
		...
		$data = "";
		foreach($_POST as $k => $v){
			if(!in_array($k,array('id','senin','old_file'))){
				if(!empty($data)) $data .= ", ";
				$data .= "`$k` = '$v'";
			}
		}
				if(!empty($data)) $data .= ", ";
				$data .= "`senin` = '".addslashes(htmlentities($senin))."'";
		...
		if(empty($id)){
			...
			$sql ="UPDATE doctors set $data where id = {$id}";
		}
		$save = $this->conn->query($sql);
		$action = empty($id) ? "added":"updated";
		if($save){
	...
	public function client_delete(){
		extract($_POST);
		$fpath = $this->conn->query("SELECT file_path FROM doctors where id = $id")->fetch_array()['file_path'];
